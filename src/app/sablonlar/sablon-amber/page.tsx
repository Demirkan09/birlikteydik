...
const defaultConfig = {
  ...
  musicUrl: "/music/siyah.mp3", // Müzik dosyası yolu
...
function HeartsCanvas() {
  ...
  useEffect(() => {
    ...
    particlesRef.current = Array.from({ length: 25 }, () => ({
      ...
      type: Math.random() > 0.45 ? "ember" : "heart",
    }));
    ...
    const drawAmberSpark = (cx: number, cy: number, size: number, opacity: number) => {
      ctx.save();
      ctx.globalAlpha = opacity;
      ...
      grad.addColorStop(0.6, "rgba(217, 119, 6, 0.3)");
    ...
    const animate = () => {
      ...
      particlesRef.current.forEach((p) => {
        ...
        if (p.type === "ember") {
          drawAmberSpark(p.x, p.y, p.size, p.opacity);
        } else {
...
function MusicVinylWidget({ isPlaying, toggleMusic }: { isPlaying: boolean; toggleMusic: () => void }) {
  ...
  return (
    <div
      ...
      style={{
        background: "linear-gradient(135deg, #0E0802 0%, #190F05 60%, #0E0802 100%)",
    ...
    >
      ...
      <div className="relative flex-shrink-0" style={{ width: 44, height: 44 }}>
        ...
        >
          ...
          >
            <div style={{ width: "3px", height: "3px", borderRadius: "50%", background: "#0E0802" }} />
          </div>
        </motion.div>
      </div>
...
export default function AmberTemplate({ config: propConfig, memories: propMemories }: { config?: any, memories?: any[] } = {}) {
  ...
  return (
    ...
    <main
      ...
      style={{
        background: "linear-gradient(160deg, #090300 0%, #150801 50%, #090300 100%)",
        color: "#FFFFFF"
      }}
    >
      {/* PARÇACIK EFEKTİ */}
      ...
      >
        ...
        <section className="relative flex flex-col items-center justify-center overflow-hidden w-full h-[100svh]">
          ...
          <div
            ...
            style={{
              background: `radial-gradient(circle at 50% 45%, rgba(245,158,11,0.22) 0%, rgba(217,119,6,0.08) 45%, transparent 70%)`,
              zIndex: 1,
            }}
          />
          {/* Ambient sunset glow orb */}
          <div
            ...
            style={{
              ...
              height: "280px",
              borderRadius: "50%",
              background: "radial-gradient(circle, rgba(251,191,36,0.18) 0%, rgba(245,158,11,0.05) 50%, transparent 80%)",
              filter: "blur(40px)",
              zIndex: 1,
            }}
          />
